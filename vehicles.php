...
function renderAdvertCard($details) {
    ...
    $placeholderIcon = $details['placeholderIcon'] ?? 'car-sport-outline';
...
<head>
    ...
    <title>Vehicles for Hire – TRIDOTS</title>
    ...
    <style>
        :root {
            ...
            --gradient: linear-gradient(90deg, #9b59b6 0%, #8e44ad 100%);
        }
...
<body>
    ...
    <main class="container">
        <h1 class="page-title">Vehicles for Hire</h1>
        <div class="ad-grid">
            <?php if (!empty($vehicles)): ?>
                <?php foreach ($vehicles as $vehicle): ?>
                    <?php renderAdvertCard([
                        'link' => $userSignedIn ? "vehicle_details.php?id=" . urlencode($vehicle['vehicle_id']) : "login.php",
                        'imageUrl' => getFirstImage($vehicle['vehicle_images'], $image_base_url),
                        'altText' => $vehicle['vehicle_name'],
                        'title' => $vehicle['vehicle_name'],
                        'location' => $vehicle['location'],
                        'priceLine' => '<p class="item-price">Rs. ' . htmlspecialchars(number_format((float)$vehicle['amount'], 2)) . ' <span>/ ' . htmlspecialchars($vehicle['price_type']) . '</span></p>'
                    ]); ?>
                <?php endforeach; ?>
            <?php else: ?>
                <div class="empty-message"><ion-icon name="car-sport-outline"></ion-icon><br>No vehicles available for hire at the moment.</div>
            <?php endif; ?>
        </div>
    </main>
