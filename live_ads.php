...
$stmt = $pdo->prepare("SELECT * FROM renting WHERE approval_status = 'Approved' ORDER BY created_at DESC");
$stmt->execute();
...
<html lang="en">
<head>
    <meta charset="UTF-8"><title>Live Ads | Admin</title>
    ...
    <div class="row flex-nowrap">
        ...
        <nav class="col-auto col-md-3 col-lg-2 px-3 sidebar d-flex flex-column">
            ...
            <ul class="nav nav-pills flex-column mb-auto">
                ...
                <li>
                    <a href="live_ads.php" class="nav-link active"><i class="bi bi-bullseye me-2"></i>Live Ads</a>
                </li>
                <li>
        ...
        <main class="col main-content">
            <div class="dashboard-title mb-3"><i class="bi bi-bullseye"></i> Live Ads</div>
            ...
            <table class="table table-hover align-middle">
                ...
                <?php foreach ($ads as $ad): ?>
                    <tr>
                        <td><?= htmlspecialchars($ad['rent_id'] ?? '') ?></td>
                        <td><?= htmlspecialchars($ad['product_name'] ?? '') ?></td>
                        <td><?= htmlspecialchars($ad['user_id'] ?? '') ?></td>
                        <td><?= htmlspecialchars($ad['brand'] ?? '-') ?></td>
                        <td>
                            <span class="badge bg-success"><?= htmlspecialchars($ad['approval_status'] ?? '') ?></span>
                        </td>
                        <td><?= htmlspecialchars($ad['created_at'] ?? '') ?></td>
                    </tr>
                <?php endforeach ?>
                <?php if (empty($ads)): ?>
                    <tr><td colspan="6" class="text-center">No live ads found.</td></tr>
                <?php endif; ?>
                </tbody>
            </table>
            </div>
