...
$stmt = $pdo->prepare("SELECT * FROM Vehicles WHERE approval_status = 'Approved' ORDER BY created_at DESC");
$stmt->execute();
...
<html lang="en">
<head>
    <meta charset="UTF-8"><title>Live Vehicles | Admin</title>
    ...
    <div class="row flex-nowrap">
        ...
        <nav class="col-auto col-md-3 col-lg-2 px-3 sidebar d-flex flex-column">
            ...
            <ul class="nav nav-pills flex-column mb-auto">
                ...
                <li>
                    <a href="live_vehicles.php" class="nav-link active"><i class="bi bi-truck me-2"></i>Live Vehicles</a>
                </li>
                <li>
        ...
        <main class="col main-content">
            <div class="dashboard-title mb-3"><i class="bi bi-truck"></i> Live Vehicles</div>
            ...
            <table class="table table-hover align-middle">
                ...
                <?php foreach ($vehicles as $vehicle): ?>
                    <tr>
                        ...
                        <td><?= htmlspecialchars($vehicle['user_id'] ?? '') ?></td>
                        <td><?= htmlspecialchars($vehicle['brand'] ?? '-') ?></td>
                        <td>
                            <span class="badge bg-success"><?= htmlspecialchars($vehicle['approval_status'] ?? '') ?></span>
                        </td>
                        <td><?= htmlspecialchars($vehicle['created_at'] ?? '') ?></td>
                    </tr>
                <?php endforeach ?>
                <?php if (empty($vehicles)): ?>
                    <tr><td colspan="6" class="text-center">No live vehicles found.</td></tr>
                <?php endif; ?>
                </tbody>
            </table>
            </div>
