...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        $stmt = $pdo->prepare("INSERT INTO Users (username, email_address, password_hash, contact_number, nic_number, user_type, address, verification_status, is_active)
                               VALUES (?, ?, ?, ?, ?, 'Personal', ?, 'Not Verified', 'Active')");
        $stmt->execute([$username, $email, $password, $contact, $nicNumber, $address]);

        echo "<script>
            alert('Congratulations! Your personal account registration was successful.');
            window.location.href = 'login.php';
        </script>";
        exit;
