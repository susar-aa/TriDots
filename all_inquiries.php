...
<html lang="en">
<head>
    <meta charset="UTF-8"><title>Inquiries | Admin</title>
    ...
    <div class="row flex-nowrap">
        ...
        <nav class="col-auto col-md-3 col-lg-2 px-3 sidebar d-flex flex-column">
            ...
            <ul class="nav nav-pills flex-column mb-auto">
                ...
                <li>
                    <a href="all_inquiries.php" class="nav-link active"><i class="bi bi-envelope me-2"></i>Inquiries</a>
                </li>
            </ul>
        ...
        <main class="col main-content">
            <div class="dashboard-title mb-3"><i class="bi bi-envelope"></i> Inquiries</div>
            ...
            <table class="table table-hover align-middle">
                ...
                <?php foreach ($inquiries as $inquiry): ?>
                    <tr>
                        ...
                        <td><?= htmlspecialchars($inquiry['ad_type'] ?? '') ?></td>
                        <td><?= htmlspecialchars($inquiry['ad_id'] ?? '') ?></td>
                        <td>
                            <span class="badge bg-<?= ($inquiry['status'] ?? '') === 'Confirmed' ? 'success' : (($inquiry['status'] ?? '') === 'Pending' ? 'warning' : 'secondary') ?>">
                                <?= htmlspecialchars($inquiry['status'] ?? '') ?>
                            </span>
                        </td>
                        <td><?= htmlspecialchars($inquiry['inquiry_date'] ?? '') ?></td>
                        <td><?= htmlspecialchars($inquiry['inquirer_name'] ?? '-') ?></td>
                        <td><?= htmlspecialchars($inquiry['inquirer_email'] ?? '-') ?></td>
                    </tr>
                <?php endforeach ?>
