...
<html lang="en">
<head>
    <meta charset="UTF-8"><title>Service Providers | Admin</title>
    ...
    <div class="row flex-nowrap">
        ...
        <nav class="col-auto col-md-3 col-lg-2 px-3 sidebar d-flex flex-column">
            ...
            <ul class="nav nav-pills flex-column mb-auto">
                ...
                <li>
                    <a href="all_service_providers.php" class="nav-link active"><i class="bi bi-briefcase me-2"></i>Service Providers</a>
                </li>
                <li>
        ...
        <main class="col main-content">
            <div class="dashboard-title mb-3"><i class="bi bi-briefcase"></i> Service Providers</div>
            ...
            <table class="table table-hover align-middle">
                ...
                <?php foreach ($providers as $provider): ?>
                    <tr>
                        <td><?= htmlspecialchars($provider['User_id'] ?? '') ?></td>
                        <td><?= htmlspecialchars($provider['seller_id'] ?? '') ?></td>
                        <td><?= htmlspecialchars($provider['name'] ?? '') ?></td>
                        <td><?= htmlspecialchars($provider['email_address'] ?? '') ?></td>
                        <td>
                            <span class="badge bg-<?= ($provider['approval_status'] ?? '') === 'Approved' ? 'success' : 'secondary' ?>">
                                <?= htmlspecialchars($provider['approval_status'] ?? '') ?>
                            </span>
                        </td>
                        <td>
                            <span class="badge bg-<?= ($provider['verification_status'] ?? '') === 'Verified' ? 'success' : 'secondary' ?>">
                ...
                <?php if (empty($providers)): ?>
                    <tr><td colspan="7" class="text-center">No service providers found.</td></tr>
                <?php endif; ?>
                </tbody>
            </table>
            </div>
